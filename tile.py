...
class RasterTiler:
    ...
    def process_tile(row, tile_outdir, src):
        ...
        preprocessed_data = np.tile(preprocessed_data, (3, 1, 1))
        profile = src.profile
        profile.update(transform=transform, count=3, width=preprocessed_data.shape[2], height=preprocessed_data.shape[1])
