...
class RasterTiler:
    ...
    def tile_raster(
        ...
        with ropen(self.raster_filepath, 'r') as src:
            ...
            raster_names = [self.raster_name] * len(tiles)
            tile_names = ['_'.join([str(t) for t in tile]) for tile in tiles]
            tile_exts = [self.raster_ext] * len(tiles)
            geo_tiles = [self.geo_tile(src, tile) for tile in tiles]
            geo_tiles_dataframe = gdf(
                {
                    'raster_name': raster_names,
